...
struct NewConversationView: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            NavigationView {
                SwipableSegment(tabs: ["Private", "Public"], rect: rect, offset: self.$offset, content: {
                    HStack(spacing: 0) {
                        ...
                        //.padding(.horizontal)
                        //.ignoresSafeArea()
                        .padding(.top, Constants.edges?.top ?? 15)
                        //.padding(.bottom, Constants.edges?.bottom ?? 15)
