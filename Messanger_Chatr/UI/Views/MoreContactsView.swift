            ...
            ScrollView(.vertical, showsIndicators: true) {
                VStack(spacing: 5) {
                    HStack(alignment: .bottom) {
                        Text("\(self.dialogModel.occupentsID.count + 1) TOTAL MEMBERS:")
                            .font(.caption)
                            .fontWeight(.regular)
                    ...
                    LazyVStack(alignment: .center, spacing: 0) {
                        ForEach(self.dialogModelMembers.indices, id: \.self) { id in
                            VStack(alignment: .trailing, spacing: 0) {
                                if id == 0 {
                                    DialogContactCell(showAlert: self.$showAlert, notiType: self.$notiType, notiText: self.$notiText, dismissView: self.$dismissView, openNewDialogID: self.$openNewDialogID, showProfile: self.$showProfile, contactID: self.dialogModel.owner, isAdmin: false, isOwner: true, currentUserIsPowerful: self.$currentUserIsPowerful, isLast: false, isRemoving: self.$isRemoving, isPublic: self.dialogModel.dialogType == "public")
